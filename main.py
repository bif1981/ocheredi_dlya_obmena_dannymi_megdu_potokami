...
# Метод serve_customer(self, customer) - моделирует обслуживание посетителя. Проверяет наличие свободных столов, в
# случае наличия стола - начинает обслуживание посетителя (запуск потока), в противном случае - посетитель поступает
...
class Cafe:
    ...
    def customer_arrival(self):
        customer_number = 1
        while customer_number <= 20:  # Можете изменить количество посетителей здесь
            print(f"Посетитель номер {customer_number} прибыл.")
            self.serve_customer(Customer(customer_number, self))
            customer_number += 1
            time.sleep(1)
